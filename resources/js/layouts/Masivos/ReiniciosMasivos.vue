						...
						<v-card v-show="expand" elevation="0">
							<v-card-text class="pb-0">
								<v-row class="">
									...
									<v-overlay absolute :value="overlay.estado" color="#5a48ea" opacity="0.60">
										<div
												...
												<v-card class="mx-auto elevation-0">
													<v-card-subtitle class="pa-0">
														<v-row>
															<v-col cols="12" md="6" lg="6" class="pt-1 text-left">
																<v-text-field
																v-model="search"
																label="Buscar usuario por DNI, nombre o apellidos"
																class=""
													...
													>
														<v-row >
															...
															<v-col cols="2" md="2" lg="2" class="text-center py-0"> DNI </v-col>
													...
													<v-card-text v-else-if="data_validar.pruebas.length > 0" class="pr-0">
														<v-virtual-scroll
															:items="filteredList"
															:item-height="50"
															:height="
														...
														>
															<template v-slot:default="{ item }">
																<v-row :style="{ color: item.resultado == 1 ? 'black' : 'black' }" style="border-bottom: 1px solid #e0e0e0;" class="d-flex align-end">
  ...
  export default {
	...
	computed: {
		cantidad_seleccionados() {
			let vue = this;
			let seleccionados = vue.data_validar.pruebas.filter((element) => element.selected == true)

			return seleccionados.length
		},
		filteredList() {
			let vue = this;
			console.log(vue.data_validar.pruebas);
			return vue.data_validar.pruebas.filter(item => {
				let nombre = item.usuario.nombre+' '+item.usuario.apellido_paterno+' '+item.usuario.apellido_materno;
				return item.usuario.dni.toLowerCase().includes(vue.search.toLowerCase()) || nombre.toLowerCase().includes(vue.search.toLowerCase());
			})
		}
  	},
  	mounted() {
  	...
  	methods: {
  		getData() {
  			...
  			axios
  				...
  				.then((res) => {
  					vue.baseData.modulos = res.data.modulos;
  					vue.baseData.escuelas = res.data.categorias;
  					vue.items.modulos = res.data.modulos;
  				})
  				.catch((err) => {
  					console.log(err);
  				});
  		...
  		buscarEscuela() {
  			...
  			let escuelas = vue.baseData.escuelas.filter((esc) => esc.config_id == vue.select.modulo);
  		...
  		buscarCursos() {
  			...
  			vue.select.tema = null;
  			axios
  				.get("/masivo/usuarios/buscarCursosxEscuela/" + vue.select.escuela)
  				.then((res) => {
  		...
  		buscarTemas() {
  			...
  			vue.select.tema = null;
  			axios
  				.get("/masivo/usuarios/buscarTemasxCurso/" + vue.select.curso)
  				.then((res) => {
  		...
  		temas() {
  			...
  			if (temaIndex != -1) vue.select.tema_txt = vue.items.temas[temaIndex].nombre;
