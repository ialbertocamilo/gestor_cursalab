...
class ReportsController extends Controller
{
    ...
    public function loadReportsQueue(Request $request): JsonResponse
    {
        ...
        $reportsTypes = Taxonomy::getDataByGroupAndType('reports', 'report');

        // Format reports data

        foreach ($reports as &$report) {
            $reportType = $reportsTypes
                ->where('code', $report['report_type'])
                ->first();
            $report['report_type'] = $reportType ? $reportType->name : '';
            $report['filters_descriptions'] =
                json_decode($report['filters_descriptions']) ?: new \stdClass();
            $report['created_at'] = date(
                'd/m/Y g:i a',
                strtotime($report['created_at'])
            );
            $report['updated_at'] = $report['updated_at']
                ? date('d/m/Y g:i a', strtotime($report['updated_at']))
                : '';
        }
