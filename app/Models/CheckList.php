...
class CheckList extends BaseModel
{
    ...
    protected $fillable = ['title', 'description', 'active', 'workspace_id'];
    ...
    protected function gridCheckList($data)
    {
        ...
        foreach ($checklists->items() as $checklist) {
            foreach ($checklist->checklist_actividades as $act) {
                ...
                $type_name = !is_null($type_id)
                    ? Taxonomy::where('id', $type_id)->first()
                    : null;
            ...
            foreach ($checklist->courses as $curso) {
                $workspace =
                    !is_null($curso->workspaces) && count($curso->workspaces)
                        ? $curso->workspaces[0]->name
                        : '';
                $school =
                    !is_null($curso->schools) && count($curso->schools)
                        ? $curso->schools[0]->name
                        : '';
    ...
    {
        $entrenador = EntrenadorUsuario::where('user_id', $alumno_id)
            ->where('active', 1)
            ->first();
        ...
        $cursos = Course::with('checklists', 'schools')
            ->whereIn('id', $cursos_ids)
            ->get();
        $checklists = collect();
        foreach ($cursos as $curso) {
            ...
            if ($curso->checklists->count() > 0) {
                foreach ($curso->checklists as $checklist) {
                    if ($checklist->active) {
                        $tax_trainer_user = Taxonomy::where(
                            'group',
                            'checklist'
                        )
                            ->where('type', 'type')
                            ...
                            ->first();
                        $tax_user_trainer = Taxonomy::where(
                            'group',
                            'checklist'
                        )
                            ->where('type', 'type')
                            ...
                            ->first();
                        $actividades_activas = $checklist->actividades
                            ->where('active', 1)
                            ->where('type_id', $tax_trainer_user->id)
                            ->sortBy('position');
                        $actividades_activasFeedback = $checklist->actividades
                            ->where('active', 1)
                            ->where('type_id', $tax_user_trainer->id)
                            ->sortBy('position');
                        if (
                            !$checklists
                                ->where('id', $checklist->id)
                                ->first() &&
                            $actividades_activas->count() > 0 &&
                            $checklist->active
                        ) {
                            $r_x_c = SummaryCourse::where('user_id', $alumno_id)
                                ->where('course_id', $curso->id)
                                ->first();

                            $aprobado = Taxonomy::getFirstData(
                                'course',
                                'user-status',
                                'aprobado'
                            );
                            ...
                            $disponible =
                                $r_x_c && $r_x_c->status_id === $aprobado->id;
                            $checklistRpta = ChecklistRpta::checklist(
                                $checklist->id
                            )
                                ->alumno($alumno_id)
                            ...
                            $progresoActividad = $this->getProgresoActividades(
                                $checklist,
                                $checklistRpta,
                                $actividades_activas
                            );
                            $progresoActividadFeedback = $this->getProgresoActividadesFeedback(
                                $checklistRpta,
                                $actividades_activasFeedback
                            );
                            $tempChecklist = [
                                'id' => $checklist->id,
                                ...
                                'curso' => $checklist
                                    ...
                                    ->get(),
                                'porcentaje' =>
                                    $progresoActividad['porcentaje'],
                                'actividades_totales' =>
                                    $progresoActividad['actividades_totales'],
                                'actividades_completadas' =>
                                    $progresoActividad[
                                        'actividades_completadas'
                                    ],
                                'actividades' =>
                                    $progresoActividad['actividades'],
                                'actividades_feedback' =>
                                    $progresoActividadFeedback[
                                        'actividades_feedback'
                                    ],
                                'feedback_disponible' =>
                                    $progresoActividad['feedback_disponible'],
                            ];
                            if ($tempChecklist['porcentaje'] === 100.0) {
        ...
        $response['porcentaje'] =
            $checklists->count() > 0
                ? (float) number_format(
                    ($checklistCompletados / $checklists->count()) * 100,
                    2
                )
                : 0;
    ...
    public function getProgresoActividadesFeedback(
        ChecklistRpta $checklistRpta,
        $actividades
    ) {
        foreach ($actividades as $actividad) {
            $actividadProgreso = ChecklistRptaItem::where(
                'checklist_item_id',
                $actividad->id
            )
                ->where('checklist_answer_id', $checklistRpta->id)
                ->first();
            ...
            $type_name = !is_null($actividad->type_id)
                ? Taxonomy::where('id', $actividad->type_id)->first()
                : null;
            $type_name = !is_null($type_name) ? $type_name->code : '';
    ...
    public function getProgresoActividades(
        Checklist $checkList,
        ChecklistRpta $checklistRpta,
        $actividades
    ) {
        ...
        if ($checklistRpta->porcentaje !== 100) {
            foreach ($actividades as $actividad) {
                $checklistRptaItem = ChecklistRptaItem::where(
                    'checklist_answer_id',
                    $checklistRpta->id
                )
                    ->where('checklist_item_id', $actividad->id)
                    ->first();
                ...
                if ($actividadProgreso) {
                    $actividad->estado = $actividadProgreso->qualification;
                    if (in_array($actividad->estado, ['Cumple', 'No cumple'])) {
                        $completadas++;
                    }
                } else {
                    $actividad->estado = 'Pendiente';
            ...
            $actividadProgreso = ChecklistRptaItem::with('rpta_items')
                ->where('checklist_answer_id', $checklistRpta->id)
                ->get();
            $actividades = collect();
