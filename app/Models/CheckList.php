...
class CheckList extends BaseModel
{
    ...
    protected function listActivities($checklist,$request){
        ...
        foreach ($_activities as $index => $activity) {
            ...
            $activities[]  = [
                ...
                'principal_comment' => $comments  ? $comments->where('user_id',$user->id)->first() : null,
                'comments' => $comments?->where('user_id',$user->id)->values() ?? [],
                'photo' => $list_photos,
        ...
        if($request->entity_id){
            $criterion_value_user_entity = CriterionValue::select('value_text','id')->where('id',$request->entity_id)->first();
        }else{
        ...
        return [
            ...
            'checklist'=>[
                ...
                "entity" =>[
                    "id" => $criterion_value_user_entity?->id,
    ...
    protected function listSupervisedChecklist($checklist,$entity,$request){
        $user = auth()->user();
        $criterion_values_id = [];
        ...
        return [
            ...
            'paginate' =>[
                'current_page' => $list_users->currentPage(),
                'last_page' => $list_users->lastPage(),
                'total' => $list_users->total()
            ]
        ];
    }
